...
func randomRouter(seed int64) routerFactory {
	return func() database.Router {
		return database.NewRandomRouter(database.Single{}, simulator.NewStreams(seed).Routing)
	}
}
...
func writeThenView(seed int64, newRouter routerFactory) []simulator.Event {
	w := simulator.NewWorld(simulator.Config{Seed: seed, Followers: 2})
	router := newRouter()
	application := w.Application(router)
	session := simulator.NewSession("alice")
...
func run(seed int64, newRouter routerFactory, cfg func(*simulator.Config)) *simulator.World {
	c := simulator.Config{Seed: seed, Followers: 3}
	if cfg != nil {
		cfg(&c)
	}
	w := simulator.NewWorld(c)
	router := newRouter()
	application := w.Application(router)
	session := simulator.NewSession("alice")
...
func dump(t *testing.T, h []simulator.Event) string {
	t.Helper()
...
func TestPartitionerSpreadsWrites(t *testing.T) {
	for _, tc := range []struct {
		...
		w := run(7, partitionedLeaderRouter(tc.p), func(c *simulator.Config) {
			c.Partitions = tc.n
		})
		used, total := 0, 0
		for part := 0; part < tc.n; part++ {
			location := database.Location{Partition: database.PartitionID(part), Node: 0}
