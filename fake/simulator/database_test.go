...
func databaseWithFollowerPositions(positions ...int) *Database {
	cluster := database.NewCluster(1, len(positions))
	records := []database.Record{
		{Post: &post.Post{ID: "p1", ThreadID: "t1", Body: "one"}},
		{Post: &post.Post{ID: "p2", ThreadID: "t1", Body: "two"}},
	}
	...
	for follower, applied := range positions {
		...
		for pos := 1; pos <= applied; pos++ {
			cluster.Apply(location, database.Position(pos), records[pos-1])
		}
	}
	return &Database{
		s:      &Sim{c: cluster},
...
func TestDatabaseReadsFromCaughtUpFollowerSelectedByRouter(t *testing.T) {
	fakeDB := databaseWithFollowerPositions(1, 2)

	result := fakeDB.ReadThread("t1", database.RequiredPositions{0: 2})
...
func TestDatabaseReadsFromLeaderWhenFollowersLag(t *testing.T) {
	fakeDB := databaseWithFollowerPositions(1, 1)
