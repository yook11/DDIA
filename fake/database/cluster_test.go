...
func TestReplicaAppliesInOrder(t *testing.T) {
	replica := newReplica()
	record := func(body string) Record { return Record{Post: &post.Post{Body: body}} }
...
func TestClusterAppendsOnlyAtSpecifiedLocation(t *testing.T) {
	...
	target := Location{Partition: 1, Node: 0}
	record := Record{Post: &post.Post{ID: "p1", ThreadID: "t1"}}
	...
	other := Location{Partition: 0, Node: 0}
